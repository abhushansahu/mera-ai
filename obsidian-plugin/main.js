...
export default class MeraContextBridgePlugin extends Plugin {
    ...
    createGrpcClient() {
        ...
        const grpcObj = loadPackageDefinition(pkgDef);
        const ServiceCtor = grpcObj.mera.sidecar.v1.SidecarContextService;
        return new ServiceCtor(this.settings.grpcAddress, credentials.createInsecure());
    }
    callGrpc(method, request) {
        if (!this.grpcClient) {
            return Promise.reject(new Error("gRPC client unavailable"));
        }
        return new Promise((resolve, reject) => {
            this.grpcClient[method](request, (error, response) => {
                if (error) {
                    reject(error);
                    return;
                }
    ...
    async refreshConnectionStatus() {
        try {
            const session = await this.callGrpc("GetSession", {
                user_id: this.settings.userId,
                space_id: this.settings.spaceId || "",
                session_id: this.settings.sessionId,
            });
...
class MeraContextBridgeSettingTab extends PluginSettingTab {
    ...
    display() {
        ...
        new Setting(containerEl)
            ...
            .onChange(async (value) => this.plugin.updateSettings({ grpcAddress: value.trim() || DEFAULT_SETTINGS.grpcAddress })));
        new Setting(containerEl)
            .setName("User ID")
            .addText((text) => text
        ...
        new Setting(containerEl)
            ...
            .onChange(async (value) => this.plugin.updateSettings({ sessionId: value.trim() || DEFAULT_SETTINGS.sessionId })));
        new Setting(containerEl)
        ...
        new Setting(containerEl)
            ...
            .addButton((button) => button.setButtonText("Send heartbeat").onClick(async () => {
            try {
